...
Compared with IID setting, the model performance in non-IID settings could drop, and the higher level of non-IID (the lower value of $\alpha$), the worse accuracy of the model. E.g., with A100 GPUs and decent method, the top-5 accuracies are	98.97%, 98.89%, 97.98%, 71.33%, and 33.67% for the $\alpha$ values of + $\infty$, 10.0, 1.0, 0.1, and 0.01, respectively. Although the performances are not good enough when $\alpha$ values are lower than or equal to 0.1, the top-5 accuracies are nearly the same as IID setting when $\alpha$ values are higher than 0.1. The All-Reduced method performs even worse than Decent under non-IID seetings. Moreover, as we can see from the figures, the learning curves under non-IID are also more unstable compared with IID. 

Such observations indicate that data heterogeneity issue is one of the main challenges when we consider appplying federated learning (and other distributed learning methods withou data sharing) to realistic autonomous driving scenario (road surface classification for this project). We will explore some client selection, local training optimization, and global aggregation enhancement methods in the future to tackle the issue.
